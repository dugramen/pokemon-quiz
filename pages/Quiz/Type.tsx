...
export default function TypeMatchup() {
  ...
  const [types, setTypes] = React.useState(blackTypes());
  ...
  const verifyGuess = (guess: string) => {
    const q =
      types[0] === types[1] ? (quiz > 1 ? 4 : quiz < 1 ? 0.25 : quiz) : quiz;
  ...
  return (
    <div className="app">
      ...
      <SwitchTransition>
        <Transition key={`${types[0]},${types[1]}`} timeout={300} mountOnEnter unmountOnExit>
          {(state) => (
            <div
              className={`question transition-all duration-300`}
              style={{
      ...
      <PkGuesser
        ...
        customAnswer={pkTypes.find((t) => verifyGuess(t)) ?? "none"}
