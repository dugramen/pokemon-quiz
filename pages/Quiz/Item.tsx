            ...
            customFetchHandler={() => {
                ...
                .then(data => {
                    setItemData({
                        ...
                        description: data.flavor_text_entries.find((entry: any) => entry.language.name === 'en').text,
                    })
                })
            }}
            verifyGuess={guess => {        
...
export async function getStaticProps() {
    ...
    const responses = await Promise.all(tags.map(tag => fetch(`https://pokeapi.co/api/v2/item-category/${tag}`)))
